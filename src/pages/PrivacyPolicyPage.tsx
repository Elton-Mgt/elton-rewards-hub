...
import { PRIVACY_POLICY, PRIVACY_LAST_UPDATE } from '../constants/privacyPolicy';

interface PrivacyPolicyPageProps {
  theme: 'white' | 'black';
}

export const PrivacyPolicyPage = ({ theme }: PrivacyPolicyPageProps) => {
    ...
    <div className={`min-h-screen pt-24 pb-20 transition-colors duration-300 ${isDark ? 'bg-black text-white' : 'bg-white text-slate-900'}`}>
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-16">
          ...
          >
            ...
            <p className={`text-lg max-w-2xl mx-auto ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>
              Votre vie privée est importante pour nous. Découvrez comment ELTON OILS COMPANY SA gère et protège vos données personnelles.
            </p>
            <p className={`mt-4 text-sm font-medium ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>
              Dernière mise à jour : {PRIVACY_LAST_UPDATE}
            </p>
          </motion.div>
        </div>
        ...
        >
          ...
          <p className="text-sm">
            Vos données sont traitées conformément à la réglementation en vigueur en Côte d'Ivoire.
          </p>
        </motion.div>
      </div>
    </div>
